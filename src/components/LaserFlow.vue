<script setup>
import { useTemplateRef, onMounted, onBeforeUnmount, watch, toRefs } from 'vue'
...
const props = defineProps({
  ...
  wispDensity: { type: Number, default: 0.8 },
  flowSpeed: { type: Number, default: 0.35 },
  verticalSizing: { type: Number, default: 3.5 },
  horizontalSizing: { type: Number, default: 3 },
  fogIntensity: { type: Number, default: 0.3 },
  fogScale: { type: Number, default: 0.25 },
  wispSpeed: { type: Number, default: 8 },
  wispIntensity: { type: Number, default: 3.5 },
  flowStrength: { type: Number, default: 0.25 },
  decay: { type: Number, default: 1.1 },
  horizontalBeamOffset: { type: Number, default: 0 },
  verticalBeamOffset: { type: Number, default: -0.5 },
  mouseSmoothTime: { type: Number, default: 0.0 },
  mouseTiltStrength: { type: Number, default: 0.01 },
  falloffStart: { type: Number, default: 1.2 },
  fogFallSpeed: { type: Number, default: 0.6 },
})
...
const mountRef = useTemplateRef('canvasRef')
...
function setSizeNow() {
  ...
  const sizeChanged = Math.abs(w - lastSize.width) > 0.5 || Math.abs(h - lastSize.height) > 0.5
  const dprChanged = Math.abs(pr - lastSize.dpr) > 0.01
  if (!sizeChanged && !dprChanged) return

  lastSize = { width: w, height: h, dpr: pr }
...
onMounted(() => {
  ...
  onMove = (ev) => updateMouse(ev.clientX, ev.clientY)
  onLeave = () => mouseTarget.set(0, 0)
  ...
  const adjustDprIfNeeded = (now) => {
    ...
    if (avgFps < lowerThresh) {
      next = clamp(currentDpr * 0.85, dprFloor, baseDpr)
    } else if (avgFps > upperThresh && currentDpr < baseDpr) {
      next = clamp(currentDpr * 1.1, dprFloor, baseDpr)
    }

    if (Math.abs(next - currentDpr) > 0.01 && now - lastDprChange > dprChangeCooldown) {
      currentDpr = next
      lastDprChange = now
      setSizeNow()
    }
...
onBeforeUnmount(() => {
  ...
  if (canvas) {
    ...
    if (onCtxRestored) canvas.removeEventListener('webglcontextrestored', onCtxRestored)
  }
